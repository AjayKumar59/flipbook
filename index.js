...
function validateAndShowPage() {
    ...
    if (name && date==obj.getValue1()) {
        document.getElementById('overlay').style.display = 'none';
        document.getElementById('popup').style.display = 'none';
        document.getElementById('welcomeMessage').innerText = "Welcome, " + toCamelCase(name) + " !";
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Audio Setup
    const bgMsc = new Audio(`./assets/mp3/OMaahi.mp3`);
    bgMsc.loop = true;
    
    const turnAudio = new Audio('./assets/mp3/page-flip.mp3');
    turnAudio.loop = false;
    
    const intrucAudio = new Audio('./assets/mp3/page-flip1.mp3');
        ...
        if (clickedPage.classList.contains('turn')) {
            ...
            setTimeout(() => {
                if ((pageIndex + 3) <= numPages) {
                    ...
                    const frontPageImgSrc = isCropped ? innerSheetArray[2 * pageIndex + 2 + 2] : innerSheetArray[pageIndex + 2];
                    const backPageImgSrc = isCropped ? innerSheetArray[2 * pageIndex + 3 + 2] : innerSheetArray[pageIndex + 3];
            ...
            setTimeout(() => {
                ...
                if ((pageIndex - 3) > 0) {
                    const pageId = `page${pageIndex - 3}`;
                    const frontPageImgSrc = isCropped ? innerSheetArray[2 * pageIndex - 4 - 4] : innerSheetArray[pageIndex - 4];
                    const backPageImgSrc = isCropped ? innerSheetArray[2 * pageIndex - 3 - 4] : innerSheetArray[pageIndex - 3];
